...
class ItemRecyclerAdapter(private val listener: OnItemClickListener): RecyclerView.Adapter<ItemRecyclerAdapter.ThingHolder>() {
    ...
    inner class ThingHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        fun bindData(item: ItemRecycler) {
            ...
            ObjectAnimator.ofFloat(itemView, View.SCALE_X, 0f, 1f).setDuration((1+adapterPosition)*500L).start()
            ObjectAnimator.ofFloat(itemView, View.SCALE_Y, 0f, 1f).setDuration((1+adapterPosition)*500L).start()
        }
